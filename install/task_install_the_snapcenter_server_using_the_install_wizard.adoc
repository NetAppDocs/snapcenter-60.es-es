...
+
Consulte el sitio Web de Microsoft para obtener información sobre la activación https://support.microsoft.com/kb/223300["Registro del instalador de Windows"^].
...
NOTE: El carácter especial «`%`" is not supported in the custom path for the repository database. If you include "`%`» en el camino, la instalación falla.
...
+
Los archivos de registro siempre aparecen (los más antiguos primero) en la carpeta %temp% del usuario administrador. Si desea redirigir las ubicaciones de registro, inicie la instalación del servidor de SnapCenter desde el símbolo del sistema ejecutando:``C:\installer_location\installer_name.exe /log"C:\"``
