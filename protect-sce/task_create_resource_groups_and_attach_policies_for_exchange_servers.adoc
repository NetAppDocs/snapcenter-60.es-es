...
NOTE: También puede crear una política haciendo clic en *image:../media/add_policy_from_resourcegroup.gif["Icono más"]*.
...
.. En la sección Configure schedules for selected policies, haga clic en *image:../media/add_policy_from_resourcegroup.gif["Icono más"]* en la columna *Configure Schedules* de la política para la que desea configurar la programación.
...
+
Para la notificación por correo electrónico, debe haber especificado los detalles del servidor SMTP ya sea mediante la GUI o el comando PowerShell `Set-SmSmtpServer`.
